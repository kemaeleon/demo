...
def populate_multi_time(reader, required_fields, gene):
...
def populate_single_time(reader, required_fields, gene):
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        with open(os.path.join(os.getcwd(), kwargs['infile'])) as csvfile:
            ...
            gene = populate_gene(reader)
            if input_type == 'tc':
                populate_multi_time(reader, required_fields, gene)

            elif input_type == 'sp':
                populate_single_time(reader, required_fields, gene)
